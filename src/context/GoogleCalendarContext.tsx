...
const defaultCtx: GoogleCalendarCtx = {
  token: null,
  isConnected: false,
  connect: () => {},
  disconnect: () => {},
...
const Ctx = createContext<GoogleCalendarCtx>(defaultCtx)

const TOKEN_KEY = 'gcal_token'
const EXPIRY_KEY = 'gcal_expiry'
...
export function GoogleCalendarProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useGoogleLogin({
    ...
    onError: () => console.warn('Google Calendar auth failed'),
...
export function NoopGoogleCalendarProvider({ children }: { children: React.ReactNode }) {
